...
export type GetSapiV1DciProductListResponse = Response<200, ResponseData<ResponseBodyData<'application/json', {
'total': number; // int
'list': ({
'id': string;
'investCoin': string;
...
export type GetSapiV1DciProductListRequestResult = RequestResult<Request, GetSapiV1DciProductListResponse>

export function getSapiV1DciProductList(requestHandler: RequestHandler, payload: GetSapiV1DciProductListPayload, config?: RequestHandlerExecutionConfig): Promise<GetSapiV1DciProductListRequestResult> {return requestHandler.execute(createRequest({...payload,
endpointSchema: getSapiV1DciProductListEndpointSchema}), config);}