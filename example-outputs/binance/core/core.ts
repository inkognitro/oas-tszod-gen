...
export function isPlainObject(value: any, isSubValue = false): value is PlainObject{
if (!isSubValue) {
if (value === null) {
return true;
}
...
export type FormDataObject = {[key: string]: undefined | string | number | boolean | Blob}

export function findMatchingSchemaContentType(actualStatus: number, actualContentType: string, endpointSchema: EndpointSchema): string | null {
...
export type RequestBody = Blob | FormData | FormDataObject | PlainObject | string

export type RequestBodyData<TContentType extends string = any,
TBody extends RequestBody = any,> = {contentType: TContentType;
body: TBody;}

export type Request<Ct extends string | undefined = any,
...
export type RequestUnion<TBodyData extends RequestBodyData,
TPathParams extends PathParams | undefined = any,
TQueryParams extends QueryParams | undefined = any,
THeaders extends RequestHeaders | undefined = any,
TCookies extends RequestCookies | undefined = any,> = TBodyData extends any ? Request<TBodyData["contentType"],
TBodyData["body"],
TPathParams,
TQueryParams,
THeaders,
...
export type RequiredAndPartial<T extends object = any,
RFields extends keyof T = any,
OFields extends keyof T = any,> = Required<Pick<T, RFields>> & Partial<Pick<T, OFields>>

export type RequestPayload<TRequest extends  Request = any,
RFields extends 'pathParams' | 'queryParams' | 'contentType' | 'body' = any,
OFields extends 'cookies' | 'headers' | 'queryParams' = any,> = RequiredAndPartial<{requestId: string;
...
body: TRequest["body"];}, RFields, "requestId" | OFields>
...
export function createRequestId(): string {const chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789".split("");
let requestId = "";
for (let i = 32; i > 0; i--) {
...
export function createRequestUrl(endpointPath: string,
params: PathParams,): string {const urlVariableNames = endpointPath.match(/[^{}]+(?=})/g) ?? [];
...
export function createRequest<TPayload extends RequestPayload>(endpointSchema: EndpointSchema, payload?: RequestPayload): RequestFromPayload<TPayload> {
const p = payload ?? {};
return {
...payload,
...
export type ResponseBody = Blob | FormData | FormDataObject | PlainObject | string

export type ResponseBodyData<ContentType extends string = any,
B extends ResponseBody = any> = {
contentType: ContentType;
body: B;
}

export interface Response<S extends number = any,
...
export type ResponseUnion<S extends number = any,
BodyData extends ResponseBodyData = any,
H extends ResponseHeaders = any,
C extends ResponseSetCookies = any> = BodyData extends any
? Response<S, BodyData['contentType'], BodyData['body'], H, C>
: never;

export interface RequestResult<Req extends Request = any,
...
execute(request: Request, config?: RequestHandlerExecutionConfig): Promise<RequestResult>
}
